...
_prompt_scene_technical_implementation = """You are an expert in educational video production and Manim (Community Edition), adept at translating visual storyboards into robust and spatially accurate Manim code.  
...
4. If external assets (e.g., images, video) are referenced, remove them.
5. Preserve all original code that is not causing the reported error. Do not remove or alter any intentional elements unnecessarily.
6. Follow best practices for code clarity and the current Manim version.
...
_prompt_context_learning_animation = """Here are some example animation plans to help guide your planning:

{examples}"""

_prompt_scene_animation = """You are an expert in educational video production and Manim animation, skilled in creating engaging and pedagogically effective learning experiences.  

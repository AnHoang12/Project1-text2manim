...
from task_generator import (
    ...
    get_prompt_rag_query_generation_animation,
...
class RAGIntegration:
    ...
    def __init__(self, helper_model, output_dir, chroma_db_path, manim_docs_path, embedding_model="sentence-transformers/all-MiniLM-L6-v2", use_langfuse=True, session_id=None):
    ...
    def _generate_rag_queries_animation(self, storyboard: str, scene_trace_id: str = None, topic: str = None, scene_number: int = None, session_id: str = None, relevant_plugins: List[str] = []) -> List[str]:
        """Generate RAG queries from the storyboard to help create animation plan.
        ...
        Returns:
            List[str]: List of generated RAG queries
        """
        cache_key = f"{topic}_scene{scene_number}_animation_rag"
        cache_dir = os.path.join(self.output_dir, re.sub(r'[^a-z0-9_]+', '_', topic.lower()), f"scene{scene_number}", "rag_cache")
        os.makedirs(cache_dir, exist_ok=True)
        cache_file = os.path.join(cache_dir, "rag_queries_animation.json")

        if os.path.exists(cache_file):
            with open(cache_file, 'r') as f:
                return json.load(f)

        prompt = get_prompt_rag_query_generation_animation(
            storyboard=storyboard,
            relevant_plugins=", ".join(relevant_plugins) if relevant_plugins else "No plugins are relevant."
        )
        ...
        queries = self.helper_model(
            _prepare_text_inputs(prompt),
            metadata={"generation_name": "rag_query_generation_animation", "trace_id": scene_trace_id, "tags": [topic, f"scene{scene_number}"], "session_id": session_id}
        )

        try:
            # First try to find JSON in triple backticks
            json_match = re.search(r'```json(.*)```', queries, re.DOTALL)
            if json_match:
                queries = json_match.group(1)
            else:
                # If no JSON in backticks, try to find any JSON-like structure
                json_match = re.search(r'\[.*\]', queries, re.DOTALL)
                if json_match:
                    queries = json_match.group(0)
                else:
                    print(f"Could not find JSON structure in response: {queries}")
                    return []

            queries = json.loads(queries)
        except json.JSONDecodeError as e:
            print(f"JSONDecodeError when parsing animation RAG queries: {e}")
            print(f"Response text was: {queries}")
            return []
        except Exception as e:
            print(f"Error processing animation RAG queries: {e}")
            print(f"Response text was: {queries}")
            return []

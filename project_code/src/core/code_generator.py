...
# from src.rag.simple_rag import SimpleRAG  # Import SimpleRAG instead of RAGVectorStore
from src.rag.vector_store import RAGVectorStore

class CodeGenerator:
    """A class for generating and managing Manim code."""

    def __init__(self, model="openai/gpt-4o", output_dir="output", print_response=False, use_rag=False, use_context_learning=False, context_learning_path="data/context_learning", chroma_db_path="rag/chroma_db", manim_docs_path="rag/manim_docs", embedding_model="azure/text-embedding-3-large", use_visual_fix_code=False, use_langfuse=True, session_id=None):
        """Initialize the CodeGenerator.

        Args:
            model (str, optional): OpenAI model to use. Defaults to "openai/gpt-4o".
        ...
        """
        self.model = OpenAIWrapper(model_name=model.split('/')[-1] if '/' in model else model)
    ...
    def visual_self_reflection(self, code: str, media_path: Union[str, Image.Image], scene_trace_id: str, topic: str, scene_number: int, session_id: str) -> str:
        """Use snapshot image to fix code.
        ...
        if isinstance(media_path, str):
            if media_path.endswith('.mp4'):
                # For video, inform in the prompt that we're using frames
                prompt += "\n\nNote: I'm providing a snapshot from the video. Please analyze this frame to identify issues."
                # For simplicity, just skip video processing and inform user
                print("Note: OpenAI doesn't support video input. Using first frame or snapshot instead.")
                # Here you would ideally extract a frame from the video
                media = Image.open(media_path) if not media_path.endswith('.mp4') else None
            else:
        ...
        if media:
            response_text = self.model(
                _prepare_text_image_inputs(prompt, media),
                metadata={
            ...
            # If media couldn't be processed, fall back to text-only
            response_text = self.model(
                _prepare_text_inputs(prompt + "\n\nNote: Unable to process the media. Please provide suggestions based on the code alone."),
            metadata={
                "generation_name": "visual_self_reflection",
                "trace_id": scene_trace_id,
                "tags": [topic, f"scene{scene_number}"],
